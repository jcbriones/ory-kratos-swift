...
**requestedAal** | [**OryAuthenticatorAssuranceLevel**](OryAuthenticatorAssuranceLevel.md) |  | [optional] 
...
**state** | **JSONValue** | State represents the state of this request:  choose_method: ask the user to choose a method to sign in with sent_email: the email has been sent to the user passed_challenge: the request was successful and the login challenge was passed. | 
**transientPayload** | **JSONValue** | TransientPayload is used to pass data from the login to hooks and email templates | [optional] 
