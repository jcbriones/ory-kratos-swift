//
// OryIdentityPatchResponse.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

/** Response for a single identity patch */
public struct OryIdentityPatchResponse: Sendable, Codable, ParameterConvertible, Hashable {

    public enum OryAction: String, Sendable, Codable, CaseIterable {
        case create = "create"
        case error = "error"
    }
    /** The action for this specific patch create ActionCreate  Create this identity. error ActionError  Error indicates that the patch failed. */
    public var action: OryAction?
    public var error: JSONValue?
    /** The identity ID payload of this patch */
    public var identity: String?
    /** The ID of this patch response, if an ID was specified in the patch. */
    public var patchId: String?

    public init(action: OryAction? = nil, error: JSONValue? = nil, identity: String? = nil, patchId: String? = nil) {
        self.action = action
        self.error = error
        self.identity = identity
        self.patchId = patchId
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case action
        case error
        case identity
        case patchId = "patch_id"
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(action, forKey: .action)
        try container.encodeIfPresent(error, forKey: .error)
        try container.encodeIfPresent(identity, forKey: .identity)
        try container.encodeIfPresent(patchId, forKey: .patchId)
    }
}

