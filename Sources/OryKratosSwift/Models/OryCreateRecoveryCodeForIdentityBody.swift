//
// OryCreateRecoveryCodeForIdentityBody.swift
//
// Generated by openapi-generator
// https://openapi-generator.tech
//

import Foundation

/** Create Recovery Code for Identity Request Body */
public struct OryCreateRecoveryCodeForIdentityBody: Sendable, Codable, ParameterConvertible, Hashable {

    public static let expiresInRule = StringRule(minLength: nil, maxLength: nil, pattern: "/^([0-9]+(ns|us|ms|s|m|h))*$/")
    /** Code Expires In  The recovery code will expire after that amount of time has passed. Defaults to the configuration value of `selfservice.methods.code.config.lifespan`. */
    public var expiresIn: String?
    /** The flow type can either be `api` or `browser`. */
    public var flowType: String?
    /** Identity to Recover  The identity's ID you wish to recover. */
    public var identityId: String

    public init(expiresIn: String? = nil, flowType: String? = nil, identityId: String) {
        self.expiresIn = expiresIn
        self.flowType = flowType
        self.identityId = identityId
    }

    public enum CodingKeys: String, CodingKey, CaseIterable {
        case expiresIn = "expires_in"
        case flowType = "flow_type"
        case identityId = "identity_id"
    }

    // Encodable protocol methods

    public func encode(to encoder: Encoder) throws {
        var container = encoder.container(keyedBy: CodingKeys.self)
        try container.encodeIfPresent(expiresIn, forKey: .expiresIn)
        try container.encodeIfPresent(flowType, forKey: .flowType)
        try container.encode(identityId, forKey: .identityId)
    }
}

